...
void Tema1::Init()
{
    ...
    Mesh* bullet = object_2D::CreateSquare(
                "bullet", glm::vec3(0, 0, 0), lineLength, glm::vec3(0.5f, 0.3f, 0), true);
...
void Tema1::Update(float deltaTimeSeconds)
{
    ...
    // Rata evadeaza daca a stat mai mult de 5 secunde sau nu a fost nimerita cu niciun glont
    if (currState == "active") {
    ...
    if (currState == "newDuck") {
        CalculateValues(); // calcul valori in functie de punctul si unghiul de aparitie
        currState = "active";
...
void Tema1::CalculateValues()
{
    ...
    duckCenter = glm::vec3(
        bodyCorner.x + (bodyLength * bodyScale.x) / 2, bodyCorner.y + duckHeight / 2, 0);

    duckHeight *= 1.5f; // utilizata la incadrarea ratei in box
}
...
{
    boxCorner = glm::vec3(duckCenter.x - duckWidth / 2, duckCenter.y - duckHeight / 2 - 45, 0);
    ...
    modelMatrix *= transform_2D::Scale(duckWidth, duckHeight);
...
void Tema1::OnMouseBtnPress(int mouseX, int mouseY, int button, int mods)
{
    if (IS_BIT_SET(button, GLFW_MOUSE_BUTTON_LEFT) && currState == "active") {
        ...
        mouse-ului pentru a avea aceste coordonate proportionale */
